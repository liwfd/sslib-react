...
function multiFilesUpLoad(obj, billType, source,callback,showWaterMark) {
    ...
    if (obj.file) {
        ...
        reader.onload = function (e) {
            ...
            let nowDate=formatUtils.formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss');
 ...
 *
 * */
function addWaterMark(imgData) {
    let nowDate=formatUtils.formatDate(new Date(), 'yyyy-MM-dd hh:mm');
    let userName = AuthToken.getContext()._A_P_userName;
    let text=`${userName} ${nowDate}`;
    ...
    let fsz = 28;
    context.font = fsz + "px Arial";
    context.textBaseline = 'middle';//更改字号后，重置对齐方式
    context.textAlign = 'center';
    let tw = context.measureText(text).width;
    let ftop = canvas.height;
    let fleft = canvas.width;
    context.fillText(text, fleft-tw/2-10, ftop-fsz);//文本元素在右下角
